...
def body_symmetry(LPEL, RPEL, LSHO, RSHO, threshold, time_vector, plot=False):
    ...
    distance_diff_right_left = np.abs(distance_shoulder_pelvis_right - distance_shoulder_pelvis_left)

    # 3. Detect contact and separation intervals
    diff_right_left_interval = get_threshold_intervals(distance_diff_right_left, threshold, "above")


    # 4. Analyze timing and duration of those intervals
    shoulder_pelvis_contact = analyze_intervals_duration(diff_right_left_interval, time_vector)
