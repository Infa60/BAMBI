...
def plot_multi_markers_speed_color(
    ...
    **xyz_mm  : keyword trajectories, e.g. RANK=array, LANK=array, …
    ...
    fig, axes = plt.subplots(nrows=n_markers + extra, figsize=(10, fig_h), sharex=True)
    ...
    axes[0].legend(
        handles=[
            Patch(color=red, label=f"> {thr} m/s (merged)"),
...
def marker_outcome(
    ...
    for k, v in vals.items():
        row[f"{k}_{marker_name}_{type_value}"] = (
            round(float(v), ndigits) if np.isfinite(v) else np.nan
        )
...
def plot_marker_trajectory_mean(
    ...
    if component_names is None:
        component_names = [f"{marker_name}_x", f"{marker_name}_y", f"{marker_name}_z"]
    ...
    for c in component_names:
        ...
        overshoot = np.where(
            df[c] > upper, df[c] - upper, np.where(df[c] < lower, lower - df[c], 0.0)
        )
    ...
    for ax, c in zip(axes, component_names):
        ...
        ax.fill_between(
            time, df[c], lower, where=df[c] < lower, color="green", alpha=0.70
        )
    ...
    axes[-1].set_xlabel("Time (s)")
    fig.suptitle(
        f"{marker_name} {data_type} – rolling window {win} frames, ±{k}σ", y=0.97
    )
