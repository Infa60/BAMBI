...
def interactive_label_peaks(signal: Sequence[float],
                            peaks_idx: Sequence[int],
                            out_path: str,
                            fs: float = 1.0):
    ...
    scat = ax.scatter(
        t[peaks_idx], signal[peaks_idx],
        c=[LABEL_COLORS[labels[i]] for i in range(N)],
        s=70, zorder=10, picker=7
    )
    sel_dot, = ax.plot([t[peaks_idx[current]]], [signal[peaks_idx[current]]],
                       'o', ms=18, mfc='none', mec='black', mew=2, zorder=15)
    ax.set_title('←/→ navigate | a=start (green), z=end (red), e=end+start (orange), d=hide, m=save')
    fig.tight_layout()
    plt.subplots_adjust(top=0.93)

    def redraw():
        # Update scatter colors and selection dot
        scat.set_color([LABEL_COLORS['hidden'] if hidden[i] else LABEL_COLORS[labels[i]] for i in range(N)])
    ...
    def valid_alternance(new_label, idx):
        ...
        valid_idx = [i for i, l in enumerate(labels) if l is not None and not hidden[i] and i < idx]
    ...
    def on_key(event):
        ...
        if event.key == 'm':
            ...
            valid_idx = [i for i, l in enumerate(labels) if l is not None and not hidden[i]]
            seq = [labels[i] for i in valid_idx]
            ...
            for lbl, idx_val in zip(seq, peaks_seq):
                if expecting == 'start' and lbl == 'start':
                    ...
                    show_warning("Export error: sequence is not valid. Check alternation!")
                    return
            df = pd.DataFrame({
