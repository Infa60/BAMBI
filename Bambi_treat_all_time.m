...
csv_tab = readtable(csv_path, 'VariableNamingRule', 'preserve');
...
%% === Loop over each BAMBI participant ===
for bambi = 1:length(bambiID_list)
    bambi_data = struct();
    bambiID = bambiID_list(bambi);
    indices = find(csv_tab.InclusionNumber == bambiID);
    laterality = csv_tab.Side(indices(1));
    bambiID_str = num2str(bambiID);

    % Find C3D files for this participant
    pattern = fullfile(base_path, ['*' bambiID_str '*.c3d']);
    bambi_files = dir(pattern);

    % Define the markers to process
    ...
    %% === Loop over each C3D file ===
    for i = 1:length(bambi_files)
        trial_data = struct();
        file = bambi_files(i).name;
        [~, file_no_ext, ~] = fileparts(file);
        disp(file_no_ext)

        filename = fullfile(base_path, file);
        ...
        bambi_data.(file_no_ext) = trial_data;
    end  

    % Store participant data
    bambi_name = matlab.lang.makeValidName(['Bambi0' bambiID_str]);
    results.(bambi_name) = bambi_data;
